    ...
    <div class="docs container">
      ...
      <div class="content">
        ...
        <p>A list of all your active capture jobs will appear beneath the form. When each job is finished, you can click
          <img style="width: 1.5em; height: 1.5em; vertical-align: top" alt="the preview button" title="Preview Capture" src="../assets/img/preview.svg">
          to preview the capture, and
          <img style="width: 1.1em; height: 1.1em" alt="the download button" title="Download Capture" src="../assets/img/download.svg">
          to download the web archive file.
        </p>

        <p>
          <img style="max-width: 100%;" src="../assets/img/dashboard-screenshot.png"
        ...
        <pre><code>curl -X DELETE -H "Authorization: Token {{ auth_token_key }}" {{ base_url + '/webhooks/53' }}</code></pre>
        ...
        <ol>
          <li>The replay system requires you to host a single JavaScript file on your server, which visitors browsers will use to install the service worker that handles playbacks. We recommend putting it in an empty directory, for example at <code>/replay/sw.js</code>. This file must be named <code>sw.js</code>, and it should contain the following:
            <pre><code>importScripts("{{ rwp_base_url }}/sw.js");</code></pre>
          </li>

          <li>Each page that will contain embedded archives should include the following script and should also set the height for the <code>replay-web-page</code> tag to ensure a minimum height. (600px is used as an example here).
            <pre><code>
        ...
        <p>The <code>source</code> attribute should point to the URL of where the WACZ file is hosted, while the <code>url</code> is the URL captured. The <code>replayBase</code> is the path that contains the <code>sw.js</code> file. (<code>/replay/</code> in this example is also the default, and therefore could be omitted.)</p>
        ...
        <p>The following archive was created using this system:</p>

        <ul>
          <li><a href="https://warcembed-demo.s3.amazonaws.com/twitter.com-2020-09-03.wacz">https://warcembed-demo.s3.amazonaws.com/twitter.com-2020-09-03.wacz</a>
          </li>
        </ul>

        <p>We'll embed a replay of the archive using the following code:</p>
        ...
        <p>A fully stand-alone example of this embed is also available on <a href="https://permafact-demo.glitch.me/">Glitch</a>.</p>
      </div>
    </div>
  </div>
</template>
